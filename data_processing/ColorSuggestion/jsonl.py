...
json_file_path = r'C:\Users\mlfav\lib\shlee\color_model\ColorSuggestion\train.jsonl'
count = 0
...
with open(load_json_file, 'r') as f:
    for line in f:
        ...
        hue_probabilities = []
        for color in HSV_H:
            probabilities = hue_probability(color, HSV, threshold=30)
            hue_probabilities.extend(probabilities)
        ...
        data['Hue_probability'] = calculate_hsv_statistics(probabilities)
        data['Hue_Log_probability'] = calculate_hsv_statistics(probabilities, True)
        ...
        data['Light_gradient'] = calculate_gradient(LAB_L)
        data['Hue_gradient'] = calculate_gradient(HSV_H)
        ...
        data['output_color'] = HSV_8[4].tolist()
